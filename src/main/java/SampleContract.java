...
import testbed.contracts.ContractSamples;

import java.util.ArrayList;


public class SampleContract extends SimpleWrapper {
    private enum Status {None, SecDef, SecDefFMF, Contract, Ticks, Done, Error};
    ...
    public SampleContract(int clientId, int ContractId, int mode) {
        m_clientId = clientId;
...
//                            obtainContract();
    ...
    private void submitContract() {
        consoleMsg("REQ: Contract " + m_contractId);

        m_status = m_contract.underComp() != null ? Status.Contract : Status.Ticks;

//        client().reqFundamentalData(m_contractId, new RfqContract(m_clientId), );placeOrder(m_contractId, m_contract, new RfqContract(m_clientId));
    ...
    public void deltaNeutralValidation(int reqId, DeltaNeutralContract underComp) {
        ...
        synchronized (m_mutex) {
            if (m_status == Status.Contract) {
                if (reqId != m_contractId) {
    ...
    public static void main(String[] args) {
        try {
            int ContractId = (int) (System.currentTimeMillis() / 1000);
            int mode = (args.length > 0) ? Integer.parseInt(args[0]) : 0;
            SampleContract ut = new SampleContract(/* clientId */ 2, ContractId, mode);
            ut.testContractQuery();
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
